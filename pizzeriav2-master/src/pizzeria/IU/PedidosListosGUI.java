...
public class PedidosListosGUI extends JPanel {
    ...
    private JPanel crearPanelSuperior() {
        ...
        JLabel lblTitulo = new JLabel("PEDIDOS LISTOS");
...
btnMarcarEntregado.setFont(new Font("Segoe UI", Font.BOLD, 11));
btnMarcarEntregado.setBackground(new Color(168, 27, 29));
btnMarcarEntregado.setForeground(Color.WHITE);
btnMarcarEntregado.setBorder(BorderFactory.createEmptyBorder(6, 15, 6, 15));
...
panel.add(btnMarcarEntregado);
    ...
    private void cargarDatos() {
        ...
        for (PedidoCocina pedido : listos) {
            // Para pedidos listos, mostramos la fecha en que se marcaron como listos
            // Si no tiene hora específica, mostramos la fecha de ingreso
            String fechaListo = pedido.getEstado().toString().equals("LISTO") ? 
    ...
    private void mostrarDetallePedido() {
        ...
        detalle.append(String.format("PEDIDO #%d - LISTO PARA ENTREGAR\n", pedidoSeleccionado.getIdPedidoCocina()));
        ...
        detalle.append("\n");
        detalle.append(" PEDIDO LISTO PARA SER ENTREGADO AL CLIENTE");
...
private void marcarEntregado() {
    int fila = tablaPedidos.getSelectedRow();
    if (fila == -1) {
        javax.swing.JOptionPane.showMessageDialog(this, "Seleccione un pedido primero.");
        return;
    }
    int idPedido = (int) modeloTabla.getValueAt(fila, 0);
    gestorCocina.marcarComoEntregado(idPedido);
    cargarDatos();
}
    /**
